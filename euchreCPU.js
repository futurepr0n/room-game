...
function isPartnerCurrentlyWinning(euchreState, cpuId, room) {
  ...
  try {
      // Dynamically load to avoid circular dependency
      if (!euchreTrickPlay) {
          euchreTrickPlay = require('./euchreTrickPlay');
      }
      
      // Use the imported function instead of the local implementation
      const winningPlay = euchreTrickPlay.getCurrentWinningPlay(euchreState);
      if (!winningPlay || !winningPlay.player) return false;
      
      // Use the imported arePartners function
      return euchreTrickPlay.arePartners(cpuId, winningPlay.player, room);
  } catch (error) {
